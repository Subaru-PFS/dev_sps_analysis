...
def fetch_sps_sequence(visit_set_id):
    sql_all = f"select iic_sequence.iic_sequence_id, sequence_type, name, comments,sps_exposure.pfs_visit_id, cmd_str, exp_type, sps_module_id, arm, notes, data_flag,time_exp_start,status from sps_exposure \
    inner join visit_set on sps_exposure.pfs_visit_id=visit_set.pfs_visit_id \
    ...
    WHERE iic_sequence.iic_sequence_id = {visit_set_id}"
    df = utils.fetch_query(opdb.OpDB.url, sql_all)
    return df

def getVisitRange(visit_set_id):
    query = f"select min(pfs_visit_id) as visitStart, max(pfs_visit_id) as visitEnd from visit_set WHERE iic_sequence_id  = {visit_set_id}"
...
def get_Visit_Set_Id(visit):
    query = f'select iic_sequence_id from visit_set WHERE pfs_visit_id = {visit}'
    df = utils.fetch_query(opdb.OpDB.url, query)
